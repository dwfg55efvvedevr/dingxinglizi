...
Large-repository review treats source code, comments, fixtures, generated content and embedded documents as untrusted data. They cannot override the effective project contract or current Task Package. Review records should store paths, hashes, bounded summaries and evidence references rather than unnecessary source excerpts. Inventory and result ingestion must expose exclusions, reject traversal/symlink escape and abnormal hard links, enforce resource limits, and stop on target/manifest drift.

`review_only` grants no business-source write authority. `review_and_fix` does not grant production, credential, external-message, dependency-install, destructive or irreversible authority; it only permits separately contracted project-local repairs already within the user's scope. Repair, re-review and final QA sessions must be separated. Static context/token estimates and unsigned session receipts are planning/integrity evidence, not proof against an actor with full local write access.
