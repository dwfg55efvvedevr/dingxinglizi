...
Application is non-overwriting and creates `orchestration/domain-lock.json` under the active control root with the exact pack version and SHA-256. Reapplying the same locked pack is idempotent; a different pack or modified destination requires explicit manual reconciliation.
